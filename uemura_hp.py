...
TE131_OCC_LEVEL_DEFAULT = 5
TE131_OCC_LEVEL_SPECS = {
    0: {
        "min": 30.0,
        "max": 90.0,
        "option_label": "0（30～90 kPa）",
        "result_label": "0",
    },
    5: {
        "min": 60.0,
        "max": 120.0,
        "option_label": "5（60～120 kPa・出荷時）",
        "result_label": "5（出荷時）",
    },
    9: {
        "min": 80.0,
        "max": 140.0,
        "option_label": "9（80～140 kPa）",
        "result_label": "9",
    },
}
...
def parse_te131_occ_level(val):
    try:
        level = int(float(val))
    except (TypeError, ValueError):
        return TE131_OCC_LEVEL_DEFAULT
    if level in TE131_OCC_LEVEL_SPECS:
        return level
    return TE131_OCC_LEVEL_DEFAULT

def te131_occ_level_spec(level):
    return TE131_OCC_LEVEL_SPECS[parse_te131_occ_level(level)]

def te131_occ_press_range(level):
    spec = te131_occ_level_spec(level)
    return spec["min"], spec["max"]

def resolve_infusion_occ_press_range(device_model, min_press, max_press, occ_level=None):
    if is_te131_pump(device_model):
        return te131_occ_press_range(occ_level)
    return min_press, max_press

def occlusion_item_spec(device_model, min_press, max_press, press_unit, occ_level=None):
    """閉塞圧点検の表示名・注記・入力ラベル"""
    if is_te131_pump(device_model):
        level = parse_te131_occ_level(occ_level)
        min_press, max_press = te131_occ_press_range(level)
        return {
            "name": "閉塞検出圧",
            "note": f"※テルモ汎用／ポンプ専用輸液セット レベル{level} {min_press:.0f}～{max_press:.0f} kPa",
            "caption": "閉塞検出圧 ※流量120ml/h・テルモ汎用／ポンプ専用輸液セット",
            "input_label": f"閉塞検出圧 ({press_unit})",
        }
    return {
        "name": "閉塞検出",
...
def is_bp88_vsm(device_model):
    """Nihon Kohden BP-88 / BP-88S など"""
    model = clean_data_str(device_model).upper().replace(" ", "").replace("－", "-")
    compact = model.replace("-", "")
    return "BP-88" in model or compact.startswith("BP88")

def vsm_appearance_items(device_model=""):
    if is_bp88_vsm(device_model):
        return [item for item in VSM_APPEARANCE_ITEMS if item != "マルチコネクタの破損"]
    return list(VSM_APPEARANCE_ITEMS)

def vsm_transmitter_items(device_model=""):
    if is_bp88_vsm(device_model):
        return []
    return list(VSM_TRANSMITTER_ITEMS)

def vsm_ibp_items(device_model=""):
    if is_bp88_vsm(device_model):
        return []
    return list(VSM_IBP_ITEMS)

def _vsm_all_check_labels(device_model=""):
    return (
        vsm_appearance_items(device_model) + VSM_FUNCTION_ITEMS + VSM_BATTERY_CHECK_ITEMS
        + vsm_transmitter_items(device_model) + VSM_ECG_RESP_ITEMS + VSM_SPO2_ITEMS
        + VSM_NIBP_ITEMS + vsm_ibp_items(device_model)
    )

def default_vsm_checks(device_model=""):
    return {label: "---" for label in _vsm_all_check_labels(device_model)}
...
def render_vsm_inspection_fields(vsm_checks, vsm_measurements, vsm_meta, device_model=""):
    """ベッドサイドモニタ 定期点検表の入力欄"""
    st.caption(INSPECTION_CHECK_LEGEND)
    if is_bp88_vsm(device_model):
        st.caption("対象機種: BP-88 / BP-88S（IBP・送信機・マルチコネクタ項目は対象外）")
    else:
        st.caption("対象機種例: PVM-2701 , PVM-4761")
    appearance_items = vsm_appearance_items(device_model)
    transmitter_items = vsm_transmitter_items(device_model)
    ibp_items = vsm_ibp_items(device_model)
    m1, m2 = st.columns(2)
    with m1:
    ...
    st.write("**外観点検**")
    _render_inspection_check_grid(appearance_items, vsm_checks, "vsm_app")
    ...
    if transmitter_items:
        st.write("**送信機**")
        _render_inspection_check_grid(transmitter_items, vsm_checks, "vsm_tx")
    ...
    if ibp_items:
        st.write("**IBP**")
        _render_inspection_check_grid(ibp_items, vsm_checks, "vsm_ibp")
...
def build_vsm_report_sections(vsm_checks, vsm_measurements, vsm_meta, device_model=""):
    c = vsm_checks
    m = vsm_measurements
    meta = vsm_meta or {}
    appearance_items = vsm_appearance_items(device_model)
    ...
    sections = [
        ...
        },
        {
            "title": "外観点検",
            "kind": "check",
            "items": [_check_item(l, c.get(l, "---")) for l in appearance_items],
        },
        {
            "title": "機能点検",
            "kind": "check",
    ...
    if transmitter_items:
        sections.append({
            "title": "送信機",
            "kind": "check",
            "items": [_check_item(l, c.get(l, "---")) for l in transmitter_items],
        })
    sections.extend([
        {
            "title": "ECG・RESP",
    ...
    if ibp_items:
        sections.append({
            "title": "IBP",
            "kind": "check",
            "items": [_check_item(l, c.get(l, "---")) for l in ibp_items],
        })
    sections.append({
        "title": "漏れ電流",
        "kind": "measure",
...
def build_infusion_pump_report_sections(chk_e1, chk_e2, chk_e3, chk_e4, chk_e5, chk_e6, chk_e7,
    ...
    occ_spec = occlusion_item_spec(device_model, min_press, max_press, press_unit, occ_level)
    measure_items = [
        _measured_item(
            "流量精度",
    ...
    if is_te131_pump(device_model):
        level_spec = te131_occ_level_spec(occ_level)
        measure_items.append(
            _measured_item(
                "閉塞圧レベル",
                "※テルモ汎用／ポンプ専用輸液セット 代表設定値 0・5（出荷時）・9",
                "0 / 5（出荷時） / 9",
                level_spec["option_label"],
                "〇",
            )
        )
    measure_items.append(
...
def build_inspection_report_sections(check_type, device_category, inc_o_checks,
    ...
    if device_category == "生体情報モニタ":
        return build_vsm_report_sections(
            vsm_checks or default_vsm_checks(device_model),
    ...
    if device_category in ["輸液ポンプ", "シリンジポンプ"]:
        ...
        if result == "使用可":
            ...
            if not (min_press <= occ_press <= max_press):
                occ_name = occlusion_item_spec(
                    device_model, min_press, max_press, press_unit, occ_level,
                )["name"]
        ...
        applicable_checks = {
            label: (vsm_checks or {}).get(label, "---")
            for label in _vsm_all_check_labels(device_model)
        }
        _validate_check_dict(applicable_checks, ng_items, incomplete_items)
        if result == "使用可":
    ...
    if check_type == "院内点検(miratech)":
        if device_category in ["輸液ポンプ", "シリンジポンプ"]:
            ...
            if device_category == "輸液ポンプ" and is_te131_pump(device_model):
                level_spec = te131_occ_level_spec(occ_level)
                rows.append(("閉塞圧レベル", level_spec["option_label"], "〇"))
...
def build_inspection_detail_text(check_type, device_category, result, inc_o_checks,
    ...
    if check_type == "院内点検(miratech)" and device_category in ["輸液ポンプ", "シリンジポンプ"]:
        ...
        if device_category == "輸液ポンプ" and is_te131_pump(device_model):
            parts_list.append(f"閉塞圧レベル:{parse_te131_occ_level(occ_level)}")
...
def _deserialize_inspection_draft(raw_json):
    ...
    for num_key in ("flow_acc", "occ_press", "occ_time", "occ_level", "bubble_ad_water", "bubble_ad_dry",
                    "min_flow", "max_flow", "min_press", "max_press"):
        if num_key in data:
            try:
...
def apply_inspection_draft_to_state(draft):
    ...
    for key in (
        ...
        "chk_e1", "chk_e2", "chk_e3", "chk_e4", "chk_e5", "chk_e6", "chk_e7",
        "flow_acc", "occ_press", "occ_time", "occ_level", "bubble_ad_water", "bubble_ad_dry",
...
with tabs[1]:
    ...
    occ_press = 0.0
    occ_time = 0.0
    occ_level = TE131_OCC_LEVEL_DEFAULT
    ...
    if master_row is not None:
        ...
        if restored_draft:
            ...
            if applied_draft.get("occ_level") is not None:
                occ_level = parse_te131_occ_level(applied_draft.get("occ_level"))
        ...
        if check_type == "院内点検(miratech)":
            ...
            if device_category == "輸液ポンプ":
                ...
                with col_num2:
                    occ_spec = occlusion_item_spec(
                        device_model, min_press, max_press, press_unit, occ_level,
                    )
                    st.caption(occ_spec["caption"])
                    if is_te131_pump(device_model):
                        if "inp_occ_level" not in st.session_state:
                            st.session_state["inp_occ_level"] = parse_te131_occ_level(occ_level)
                        occ_level = st.selectbox(
                            "閉塞圧レベル",
                            options=list(TE131_OCC_LEVEL_SPECS.keys()),
                            format_func=lambda lv: TE131_OCC_LEVEL_SPECS[lv]["option_label"],
                            key="inp_occ_level",
                            help="テルモ汎用／ポンプ専用輸液セット使用時の代表設定値",
                        )
                        min_press, max_press = te131_occ_press_range(occ_level)
