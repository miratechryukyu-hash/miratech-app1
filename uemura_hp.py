            ...
            if not final_me_no:
                ...
                if check_type == "院内点検(miratech)" and result == "使用可":
                    if not (min_flow <= flow_acc <= max_flow):
                        st.error(f"🚨 アラーム：流量精度（{flow_acc} {flow_unit}）が基準値外です！【使用可】での登録はできません。")
                        has_error = True
                    if not (min_press <= occ_press <= max_press):
                        st.error(f"🚨 アラーム：閉塞検出圧（{occ_press} {press_unit}）が基準値外です！【使用可】での登録はできません。")
                        has_error = True
                    if status_exterior == "NG" or status_alarm == "NG" or status_operation == "NG":
                        st.error("🚨 アラーム：点検項目に「NG」があります！【使用可】での登録はできません。")
                        has_error = True

                if has_error:
                    st.error("⚠️ データの保存を中断しました。実測数値を再調整するか、総合評価を「メーカー修理」に変更してください。")
                else:
                    try:
                        # 判定テキストの組み立て
                        ...
                        detail_text = f"【{check_type}】 外観:{status_exterior} / 警報:{status_alarm} / 作動:{status_operation} / 流量精度:{flow_acc}{flow_unit}({flow_judge}) / 閉塞圧:{occ_press}{press_unit}({press_judge})"

                        # ==========================================
                        # 印刷用・履歴用のシンプルHTML点検表の自動生成
                        # ==========================================
...
<tr>
<td style="border: 1px solid black; padding: 6px; text-align: left;">1. 外観・筐体点検</td>
...
<tr>
<td style="border: 1px solid black; padding: 6px; text-align: left;">2. 各種警報・アラーム作動試験</td>
<td style="border: 1px solid black; padding: 6px; font-weight: bold; color: {'red' if status_alarm=='NG' else 'black'};">{status_alarm}</td>
</tr>
<tr>
<td style="border: 1px solid black; padding: 6px; text-align: left;">3. 通電・基本作動点検</td>
...
<tr>
<td style="border: 1px solid black; padding: 6px; text-align: left;">閉塞検出圧</td>
